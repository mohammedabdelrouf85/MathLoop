...
export function generateQuestion(level, streak = 0, adaptiveFactor = 1.0) {
  ...
  if (effectiveLevel <= 5) {
    ...
    if (rand < 0.4) type = 'add';
    else if (rand < 0.7) type = 'sub';
    else type = 'mul';
  } else if (effectiveLevel <= 25) {
    if (rand < 0.3) type = 'add';
    else if (rand < 0.55) type = 'sub';
    else if (rand < 0.8) type = 'mul';
    else type = 'div';
  } else {
    // Level 26+: All operations + Algebra
    if (rand < 0.2) type = 'add';
    else if (rand < 0.4) type = 'sub';
    else if (rand < 0.65) type = 'mul';
    else if (rand < 0.85) type = 'div';
    else type = 'algebra';
  }
  ...
  switch (type) {
    case 'add': {
      const maxVal = Math.min(100, 10 + effectiveLevel * 4);
    ...
    case 'mul': {
      const maxMultiplier = Math.min(15, 3 + Math.floor(effectiveLevel / 3));
    ...
    case 'div': {
      ...
      const maxDivisor = Math.min(12, 2 + Math.floor(effectiveLevel / 4));
      num2 = Math.floor(Math.random() * maxDivisor) + 2; // divisor
      const maxQuotient = Math.min(15, 3 + Math.floor(effectiveLevel / 3));
    ...
    case 'algebra': {
      ...
      if (algType < 0.4) {
        // X + A = B
        const a = Math.floor(Math.random() * (10 + effectiveLevel)) + 5;
        answer = Math.floor(Math.random() * (15 + effectiveLevel)) + 2;
        const b = answer + a;
        promptText = `X + ${a} = ${b}`;
      } else if (algType < 0.7) {
        // X - A = B
        const a = Math.floor(Math.random() * (10 + effectiveLevel)) + 3;
        answer = Math.floor(Math.random() * (15 + effectiveLevel)) + 5;
        const b = answer - a;
        promptText = `X − ${a} = ${b}`;
      } else {
        // kX = B
